...
function displayBook(books) {
    books.forEach(book => {
        ...
        favBtn.className = "mt-4 px-4 py-2 bg-blue-500 text-white rounded-full hover:bg-blue-600 transition w-full"
        favBtn.textContent = "Ajouter aux favoris"
        favBtn.addEventListener("click", () => {
            let bookData = {
                book_id: book.id,
                title: book.volumeInfo.title,
                author: book.volumeInfo.authors?.join(", ") || "Auteur non disponible",
            ...
            fetch("../config/favoris.php", {
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        favBtn.classList.replace("bg-blue-500", "bg-green-500");
                    } else {
                        alert(data.error);
                    }
                })
